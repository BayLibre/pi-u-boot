...
extern int ufs_prepare_dev_for_flash(int index);
...
static ulong read_boot_storage_spinor(ulong byte_addr, ulong byte_size, void* buff)
{
	...
	/* MTD partition enumeration is a U-Boot-proper concern; the SPL reads
	 * raw NOR offsets straight through UCLASS_SPI_FLASH and has no MTD.
	 */
...
static const struct boot_storage_op storage_op[] = {
	...
	/*
	 * UFS save/restore needs scsi_scan(), which is not pulled into the SPL
	 * (no CONFIG_SPL_SCSI to keep the SPL within the SRAM budget). Quick
	 * boot on UFS boards therefore restores via NOR only in the SPL.
	 */
#if CONFIG_IS_ENABLED(SCSI)
...
static bool write_block_device(struct blk_desc *dev_desc, ulong byte_addr, ulong byte_size, void *buff)
{
	...
	pr_info("write %ldbyte to block device(%d) @address %ld\n",
		byte_size, dev_desc->uclass_id, byte_addr);
...
int board_init(void)
{
	...
	/* Probing the uclass autosets every always-on / boot-on regulator. */
	...
	if (ret) {
		...
		debug("eSPI not ready, skipping EC probe\n");
	} else {
...
#if CONFIG_IS_ENABLED(RT7451_RETIMER)
		/* The retimer is reached over the EC's I2C tunnel */
		ret = uclass_get_device_by_driver(UCLASS_MISC,
...
void run_fastboot_command(void)
{
	if (BOOT_MODE_USB == get_boot_mode() || reboot_config == BOOT_MODE_USB) {
		...
		/*
		 * Android partitions (boot/super/vbmeta...) live on UFS/SCSI; scan
		 * and prepare it so fastboot 'flash <part>' can resolve the GPT.
		 */
		if (k3_prepare_scsi_flash_target(0))
			pr_debug("scsi/ufs flash target not ready (continuing)\n");

#if !defined(CONFIG_XPL_BUILD) && CONFIG_IS_ENABLED(FASTBOOT_CMD_OEM_SPEED)
...
int board_late_init(void)
{
	...
	/*
	 * i2c2's "default" pinctrl does not actually mux the bus pads here (the
	 * TLV EEPROM probe muxes them by hand for the same reason), so set the
	 * bus-2 SCL/SDA pads to i2c mode (MUX_MODE5) before probing the fan.
	 */
	writel(0xc045, (void __iomem *)0xd401e0b8);
	writel(0xc045, (void __iomem *)0xd401e0bc);
	ret = uclass_get_device_by_driver(UCLASS_MISC,
					  DM_DRIVER_GET(ctf2301), &dev);
	if (ret && ret != -ENODEV)
		debug("Warn: Failed to force init ctf2301 fan (err=%d)\n", ret);
#endif
...
int dram_init(void)
{
	...
	gd->ram_base = CFG_SYS_SDRAM_BASE - SEC_IMG_SIZE;
...
int dram_init_banksize(void)
{
	...
	dram_size = gd->ram_base + gd->ram_size - (CFG_SYS_SDRAM_BASE - SEC_IMG_SIZE);
	...
	if (!is_power_of_2(dram_size)) {
		...
		gd->bd->bi_dram[1].start = CFG_SYS_SDRAM_BASE - SEC_IMG_SIZE + roundup_pow_of_two(dram_size / 2);
...
phys_addr_t board_get_usable_ram_top(phys_size_t total_size)
{
	...
	dram_size = memory_top - (CFG_SYS_SDRAM_BASE - SEC_IMG_SIZE);
...
/*
 * Pick the control devicetree. The BROM enters the SPL with an arbitrary,
 * non-devicetree value in a1 (saved to gd->arch.firmware_fdt_addr); under the
 * USB-download (DFU) boot path it points at unmapped memory, so dereferencing
 * any non-zero value would fault before the console is up.
 *
 * In the SPL the only devicetree is the one appended to the image (already
 * located via OF_SEPARATE), so never touch a1. In U-Boot proper the prior
 * stage (OpenSBI) hands off a real devicetree in a1; honor it when valid,
 * otherwise fall back to the appended blob.
 */
int board_fdt_blob_setup(void **fdtp)
{
	...
	if (gd->arch.firmware_fdt_addr &&
	    !fdt_check_header((void *)(uintptr_t)gd->arch.firmware_fdt_addr)) {
...
int ft_board_setup(void *blob, struct bd_info *bd)
{
	...
	if (CONFIG_IS_ENABLED(FDT_SIMPLEFB)) {
		...
		mem.start = gd_video_bottom();
		mem.end = gd_video_top() - 1;
