 ...
 *
 * Backported to U-Boot 2022.10 for Spacemit K1
 */
 ...
 * @boot_img_size: Total boot image size in bytes
 * @vendor_boot_img_size: Total vendor boot image size in bytes
 */
struct android_priv {
...
static int scan_boot_part(struct udevice *blk, struct android_priv *priv)
{
	...
	printf("ANDROID: scan_boot_part: looking for '%s'\n", partname);

	ret = part_get_info_by_name(desc, partname, &partition);
	if (ret < 0) {
		printf("ANDROID: partition '%s' not found: %d\n", partname, ret);
		return log_msg_ret("part info", ret);
	}

	printf("ANDROID: partition '%s' found at blk %lu, size %lu\n",
	       partname, (ulong)partition.start, (ulong)partition.size);

	num_blks = DIV_ROUND_UP(sizeof(struct andr_img_hdr), desc->blksz);
	bufsz = num_blks * desc->blksz;
	buf = memalign(ARCH_DMA_MINALIGN, bufsz);
	if (!buf)
		return log_msg_ret("buf", -ENOMEM);

	printf("ANDROID: reading %lu blks from %lu (blksz=%lu)\n",
	       num_blks, (ulong)partition.start, (ulong)desc->blksz);

	ret = blk_dread(desc, partition.start, num_blks, buf);
	if (ret != num_blks) {
		printf("ANDROID: blk_dread returned %d, expected %lu\n",
		       ret, num_blks);
		free(buf);
		return log_msg_ret("part read", -EIO);
	}

	printf("ANDROID: read OK, magic: %.*s\n", 8, buf);

	if (!is_android_boot_image_header(buf)) {
		printf("ANDROID: not a valid boot image header\n");
		free(buf);
		return log_msg_ret("header", -ENOENT);
	}

	if (!android_image_get_bootimg_size(buf, &priv->boot_img_size)) {
		printf("ANDROID: failed to get boot image size\n");
		free(buf);
		return log_msg_ret("get bootimg size", -EINVAL);
	}

	priv->header_version = ((struct andr_img_hdr *)buf)->header_version;
	printf("ANDROID: boot image: version=%u total_size=%u\n",
	       priv->header_version, priv->boot_img_size);
...
static int scan_vendor_boot_part(struct udevice *blk, struct android_priv *priv)
{
	...
	printf("ANDROID: scan_vendor_boot_part: looking for '%s'\n", partname);
	...
	printf("ANDROID: reading %lu blks from %lu\n",
	       num_blks, (ulong)partition.start);
	...
	if (!is_android_vendor_boot_image_header(buf)) {
		printf("ANDROID: not a valid vendor boot image header\n");
		free(buf);
		return log_msg_ret("header", -ENOENT);
	}

	if (!android_image_get_vendor_bootimg_size(buf, &priv->vendor_boot_img_size)) {
		printf("ANDROID: failed to get vendor boot image size\n");
		free(buf);
		return log_msg_ret("get vendor bootimg size", -EINVAL);
	}

	printf("ANDROID: vendor_boot image: size=%u\n",
...
static int android_read_slot_from_bcb(struct bootflow *bflow, bool decrement)
{
	...
	printf("ANDROID: read_slot_from_bcb: decrement=%d\n", decrement);

	if (!CONFIG_IS_ENABLED(ANDROID_AB)) {
		printf("ANDROID: A/B disabled, no slot\n");
		priv->slot = NULL;
		return 0;
	}
...
static int android_read_bootflow(struct udevice *dev, struct bootflow *bflow)
{
	...
	printf("ANDROID: read_bootflow: mmc %d\n", desc->devnum);
	...
	ret = part_get_info_by_name(desc, BCB_PART_NAME, &misc);
	if (ret < 0) {
		printf("ANDROID: misc partition not found: %d\n", ret);
		return log_msg_ret("part", ret);
	}

	printf("ANDROID: misc partition found, loading BCB\n");

	ret = bcb_find_partition_and_load("mmc", desc->devnum,
					(char *)BCB_PART_NAME);
	if (ret < 0) {
		printf("ANDROID: BCB load failed: %d\n", ret);
		return log_msg_ret("bcb load", ret);
	}

	ret = bcb_get(BCB_FIELD_COMMAND, command, sizeof(command));
	if (ret < 0) {
		printf("ANDROID: BCB get command failed: %d\n", ret);
		return log_msg_ret("bcb read", ret);
	}

	printf("ANDROID: BCB command='%s'\n", command);
	...
	printf("ANDROID: boot_mode=%d\n", priv->boot_mode);

	/* For recovery and normal boot, we need to scan the partitions */
	ret = android_read_slot_from_bcb(bflow, false);
	if (ret < 0) {
		printf("ANDROID: read slot failed: %d\n", ret);
		goto free_priv;
	}

	ret = scan_boot_part(bflow->blk, priv);
	if (ret < 0) {
		printf("ANDROID: scan boot failed: err=%d\n", ret);
		goto free_priv;
	}

	printf("ANDROID: boot hdr_ver=%u size=%u\n",
	       priv->header_version, priv->boot_img_size);

	if (priv->header_version >= 3) {
		ret = scan_vendor_boot_part(bflow->blk, priv);
		if (ret < 0) {
			printf("ANDROID: scan vendor_boot failed: err=%d\n", ret);
			goto free_priv;
		}
		printf("ANDROID: vendor_boot size=%u\n", priv->vendor_boot_img_size);
	}
	...
	if (priv->boot_mode == ANDROID_BOOT_MODE_NORMAL && priv->slot) {
		ret = bootflow_cmdline_set_arg(bflow, "androidboot.force_normal_boot",
					       "1", false);
		if (ret < 0) {
			printf("ANDROID: cmdline set force_normal_boot failed: %d\n", ret);
			goto free_priv;
		}
	}
	...
	printf("ANDROID: bootflow READY, os='%s' cmdline='%s'\n",
	       bflow->os_name ? bflow->os_name : "(null)",
	       bflow->cmdline ? bflow->cmdline : "(null)");

	return 0;

 free_priv:
	printf("ANDROID: read_bootflow FAILED: ret=%d\n", ret);
	free(priv);
...
static int append_bootargs_to_cmdline(struct bootflow *bflow)
{
	...
	/*
	 * Check any additional bootargs coming from U-Boot env. If any,
