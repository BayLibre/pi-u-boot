 ...
 *
 * Aligned with upstream U-Boot master for Spacemit K1 backport.
 * Adaptations for 2022.10:
 *   - image_header_t -> struct image_header
 *   - CONFIG_XPL_BUILD -> CONFIG_SPL_BUILD
 *   - CONFIG_ANDROID_BOOT_IMAGE_IGNORE_BLOB_ADDR not available
 */
...
{
	return !memcmp(VENDOR_BOOT_MAGIC, hdr, ANDR_VENDOR_BOOT_MAGIC_SIZE);
}
			     ...
			     ulong *os_data, ulong *os_len)
{
	...
	ihdr = (const image_header_t *)img_data.kernel_ptr;
...
static bool android_image_get_dtb_img_addr(ulong hdr_addr, ulong vhdr_addr, ulong *addr)
{
	...
	if (hdr->header_version > 2) {
		...
		/* Calculate the address of DTB area in vendor boot image.
		 * Must use ALIGN(header_size, page_size) for the header,
		 * not just page_size, because the header may be larger
		 * than one page (e.g. v4 header=2128 with page_size=2048).
		 */
