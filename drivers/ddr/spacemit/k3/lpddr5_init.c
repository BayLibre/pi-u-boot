...
static void build_lpddr5_io_para(const ddr_config_t* io_para, ddr_part_info* part_info)
{
	...
	/*
	 * Zero io_override_table with the D-cache disabled. On the X100 a cacheable
	 * write-allocate burst to the boot SRAM stalls during the SPL (coherent
	 * snoops to the CCI-550 are not serviced this early); uncached stores skip
	 * the allocate. dcache_{disable,enable} are leaf, no gd/console between them.
	 */
	{
		extern void dcache_disable(void);
		extern void dcache_enable(void);
		volatile unsigned long *p = (volatile unsigned long *)(uintptr_t)io_override_table;
		unsigned z;

		dcache_disable();
		for (z = 0; z < sizeof(io_override_table) / sizeof(unsigned long); z++)
			p[z] = 0;
		dcache_enable();
	}
