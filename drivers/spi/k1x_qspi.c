...
static int k1x_qspi_claim_bus(struct udevice *dev)
{
	...
	qspi->cs_selected = slave_plat->cs[0];
...
static int k1x_qspi_ofdata_to_platdata(struct udevice *bus)
{
    ...
    if (ret) {
		/*
		 * The reset controller is kept out of the SPL DT for size; the
		 * BROM already took the qspi out of reset, so continue without
		 * the reset framework instead of failing the probe.
		 */
		dev_dbg(bus, "resets not available, continuing without them\n");
